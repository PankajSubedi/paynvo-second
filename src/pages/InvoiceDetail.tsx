...
// import { Download, ArrowLeft, Mail, CheckCircle2, Pencil, Trash2 } from "lucide-react";
...
// // PDF Format configurations (width x height in mm)
// const PDF_FORMATS = {
...
//       // Create a temporary container - ALWAYS use light mode for PDFs
...
//       // Adjust margins for receipt formats
...
//                   </SelectTrigger>
//                   <SelectContent>
//                     {Object.entries(PDF_FORMATS).map(([key, format]) => (
...
//                 )}
//                 {company?.tax_id && (
//                   <p className="text-muted-foreground">Tax ID: {company.tax_id}</p>
//                 )}
//               </div>
//             </div>
...
//           {/* Invoice Details */}
//           <div className={`grid gap-4 mb-8 pb-8 border-b border-border ${
//             pdfFormat.includes('receipt') 
//               ? 'grid-cols-1' 
...
//               <p className="font-medium text-sm md:text-base">{invoice.currency}</p>
//             </div>
//           </div>
...
//                     <th className="text-left py-2 text-xs md:text-sm font-medium">Description</th>
//                     <th className="text-right py-2 text-xs md:text-sm font-medium">Qty</th>
//                     <th className="text-right py-2 text-xs md:text-sm font-medium">Price</th>
//                     <th className="text-right py-2 text-xs md:text-sm font-medium">Total</th>
//                   </tr>
//                 </thead>
//                 <tbody>
...
//                           </div>
//                         </td>
//                         <td className="text-right py-3 text-xs md:text-sm">{item.quantity}</td>
//                         <td className="text-right py-3 text-xs md:text-sm">
//                           {invoice.currency} {item.unit_price.toFixed(2)}
//                         </td>
//                         <td className="text-right py-3 text-xs md:text-sm font-medium">
...
//                     - {invoice.currency} {Number(invoice.discount_total).toFixed(2)}
//                   </span>
//                 </div>
//               )}
...
//                   <span className="font-medium">
//                     {invoice.currency} {Number(invoice.tax_total).toFixed(2)}
//                   </span>
//                 </div>
//               )}
...
//               {invoice.custom_fee && (
//                 <div className="flex justify-between text-sm">
//                   <span className="text-muted-foreground">{invoice.custom_fee.name}</span>
//                   <span className="font-medium">
//                     {invoice.custom_fee.type === "percent"
//                       ? `${invoice.custom_fee.value}%`
//                       : `${invoice.currency} ${Number(invoice.custom_fee.value).toFixed(2)}`
//                     }
//                   </span>
//                 </div>
//               )}
...
//           {/* Payment Method */}
//           {invoice.payment_method && invoice.payment_method.type && (
...
//                     <p><span className="text-muted-foreground">Bank:</span> <strong>{invoice.payment_method.bank_name}</strong></p>
//                   )}
//                   {invoice.payment_method.account_number && (
//                     <p><span className="text-muted-foreground">Account:</span> <strong>{invoice.payment_method.account_number}</strong></p>
//                   )}
//                   {invoice.payment_method.routing_number && (
//                     <p><span className="text-muted-foreground">Routing:</span> <strong>{invoice.payment_method.routing_number}</strong></p>
//                   )}
//                   {invoice.payment_method.qr_code_url && (
//                     <div className="mt-3">
//                       <p className="text-muted-foreground mb-2">Scan to pay:</p>
//                       <img src={invoice.payment_method.qr_code_url} alt="Payment QR" className="w-40 h-40 border rounded" />
//                     </div>
//                   )}
//                 </div>
//               )}

//               {invoice.payment_method.type === "payment_link" && invoice.payment_method.payment_link && (
//                 <div className="text-sm">
//                   <p className="text-muted-foreground mb-2">Pay online:</p>
//                   <a 
//                     href={invoice.payment_method.payment_link} 
//                     target="_blank" 
//                     rel="noopener noreferrer"
//                     className="text-primary hover:underline break-all"
//                   >
//                     {invoice.payment_method.payment_link}
//                   </a>
//                 </div>
//               )}
...
//                 <div className="space-y-2 text-sm">
//                   {invoice.payment_method.wallet_name && (
//                     <p><span className="text-muted-foreground">Wallet:</span> <strong>{invoice.payment_method.wallet_name}</strong></p>
//                   )}
//                   {invoice.payment_method.wallet_phone && (
//                     <p><span className="text-muted-foreground">Account:</span> <strong>{invoice.payment_method.wallet_phone}</strong></p>
//                   )}
//                   {invoice.payment_method.qr_code_url && (
//                     <div className="mt-3">
//                       <p className="text-muted-foreground mb-2">Scan to pay:</p>
        ...
        <Card className="p-6 md:p-8 shadow-soft">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8 pb-8 border-b border-border">
            <div>
              ...
              <div className="text-sm space-y-1"><p className="font-medium">{company?.name}</p>{company?.address && <p className="text-muted-foreground whitespace-pre-line">{company.address}</p>}{company?.tax_id && <p className="text-muted-foreground">Tax ID: {company.tax_id}</p>}</div>
            </div>
            <div>
              <h3 className="font-semibold mb-2">Bill To</h3>
              <div className="text-sm space-y-1"><p className="font-medium">{invoice.clients?.name || "No client"}</p>{invoice.clients?.email && <p className="text-muted-foreground">{invoice.clients.email}</p>}{invoice.clients?.billing_address && <p className="text-muted-foreground whitespace-pre-line">{invoice.clients.billing_address}</p>}{invoice.clients?.tax_id && <p className="text-muted-foreground">Tax ID: {invoice.clients.tax_id}</p>}</div>
            </div>
          </div>
          ...
          {invoice.payment_method && invoice.payment_method.type !== 'none' && (<div className="pt-6 border-t border-border"><h3 className="font-semibold mb-3">Payment Information</h3>{invoice.payment_method.type === "bank_transfer" && (<div className="space-y-2 text-sm">{invoice.payment_method.bank_name && <p><span className="text-muted-foreground">Bank:</span> <strong>{invoice.payment_method.bank_name}</strong></p>}{invoice.payment_method.account_number && <p><span className="text-muted-foreground">Account:</span> <strong>{invoice.payment_method.account_number}</strong></p>}{invoice.payment_method.routing_number && <p><span className="text-muted-foreground">Routing:</span> <strong>{invoice.payment_method.routing_number}</strong></p>}</div>)}{invoice.payment_method.type === "wallet" && (<div className="space-y-2 text-sm">{invoice.payment_method.wallet_name && <p><span className="text-muted-foreground">Wallet:</span> <strong>{invoice.payment_method.wallet_name}</strong></p>}{invoice.payment_method.wallet_phone && <p><span className="text-muted-foreground">Account:</span> <strong>{invoice.payment_method.wallet_phone}</strong></p>}{invoice.payment_method.qr_code_url && (<div className="mt-3"><p className="text-muted-foreground mb-2">Scan to pay:</p><img src={invoice.payment_method.qr_code_url} alt="Payment QR" className="w-40 h-40 border rounded" /></div>)}</div>)}</div>)}
